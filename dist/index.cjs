...
const packageFilterBuilder = (opts = {}) => {
  var _opts$conditions, _opts$fields;

  const conditions = (_opts$conditions = opts.conditions) !== null && _opts$conditions !== void 0 ? _opts$conditions : ["style", "import", "require"];
  const fields = (_opts$fields = opts.fields) !== null && _opts$fields !== void 0 ? _opts$fields : ["style", "module", "main"];
  return pkg => {
    ...
    try {
      const resolvedField = resolve_exports.legacy(pkg, {
        fields,
        browser: false
      });
    ...
    async Once(css, {
      ...
      for await (const {
        ...
        if (inline) {
          ...
          node.type = "string";
          node.value = typeof publicPath === "function" ? publicPath(node.value) : publicPath + (/[/\\]$/.test(publicPath) ? "" : "/") + path__default["default"].basename(to);
